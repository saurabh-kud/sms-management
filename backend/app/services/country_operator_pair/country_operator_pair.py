...
def create_country_operator_pair(country:str,operator:str,priority:str|None):
    ...
    res= collection.insert_one({"country": country, "operator":operator,"status":"Inactive","priority" : "High" if priority else "Low"})
    
    if res == None:
        raise CustomException(status=500,message="Error while creating country operator pair")
...
def update_country_operator_pair(country:str,operator:str,priority:str):
    ...
    res=collection.update_one({"country": country, "operator": operator}, {"$set": {"priority": priority if priority else "Low"}})
     # Check if the update affected any document
    if res.matched_count == 0:
        raise CustomException(status=500, message="Error while updating country operator pair")
    
    # Retrieve the updated document by its `_id`
    data = collection.find_one({"_id": session["_id"]})
    if data is None:
        raise CustomException(status=500, message="Error retrieving updated country operator pair")
...
def delete_country_operator_pair(country: str, operator: str):
    ...
    if res.deleted_count == 0:
        raise CustomException(status=500, message="Error while deleting country operator pair")
