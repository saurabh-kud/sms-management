...
def register(name: str, email: str, password: str, db_session: Session):
    ...
    user = DB.register(
        name=name, email=email, password=hassed_password, db_session=db_session
    )
...
def login(email: str, password: str, db_session: Session):
    ...
    hassed_password = verify_password(
        plain_password=password, hashed_password=is_already_registered.password
    )
...
async def current_user(
    token: str = Depends(oauth2_scheme), db_session: Session = Depends(get_session)
) -> User:
